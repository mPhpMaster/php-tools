 ...
 *
 * @see     RequestFilter::scopeRequestFilter()
 *
...
trait RequestFilter
{
    ...
    /**
     * @param \Illuminate\Database\Eloquent\Builder|Relation|Model $query
...
//        $inputs->dd();
    ...
    public static function applyNameFilter($query, $tables = null, Request $request = null)
    {
         ...
         * Filler by country
         */
        return $model->when(getFirstValueByKey($request, ['name'], false), function ($model, $name) use (&$inputs, $column) {
            $inputs->put('name', $name);
            return $model->where($column('name'), 'like', "%{$name}%");
        })
