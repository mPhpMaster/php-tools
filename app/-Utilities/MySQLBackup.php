...
class MySQLBackup
{
    ...
    /**
     * Set disk (default : local)
     *
     * @param  string $name       Disk namne
     * @return object MySQLBackup
     */
    ...
    public function dump()
    {
        ...
        $return .= "--\n";
        $return .= "-- By hlaCk DBTools Date: ".date("d/m/Y g:i a",time())."\n";
        $return .= "-- mr.hlaCk@gmail.com - M.F Al-Safadi \n";
        $return .= "--\n\n\n";
        ...
        \Storage::disk($this->disk)->put($this->filename.'.'.$this->extension, $return);
    ...
    private function download()
    {
        ...
        readfile(\Storage::disk($this->disk)->path($this->filename.'.'.$this->extension));
    }
        ...
        switch ($this->compressFormat)
        {
            case 'zip':
                ...
                if (class_exists('\ZipArchive'))
                {
                    ...
                    if ($zip->open($this->getPath($this->filename.'.zip'), \ZipArchive::CREATE) === true)
                    {
                        $zip->addFile($this->getPath($this->filename.'.'.$this->extension), basename($this->filename).'.'.$this->extension);
                        $zip->close();
            ...
            case 'gzip':
                ...
                $content = \Storage::disk($this->disk)->get($this->filename.'.'.$this->extension);

                \Storage::disk($this->disk)->put($this->filename.'.sql.gz', gzencode($content, 9));
    ...
    {
        if (\Storage::disk($this->disk)->exists($this->filename.'.'.$this->extension))
        {
            \Storage::disk($this->disk)->delete($this->filename.'.'.$this->extension);
        }
    }
}